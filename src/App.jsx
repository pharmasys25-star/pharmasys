...
Be thorough — scan every row/line in the image, even if the list is long. If you genuinely cannot find any drug names anywhere in the image, return []." }
      ...
      try {
        ...
        setError("AI response couldn't be read. Try a clearer or simpler photo (one list per image works best).");
        setStage("upload");
        return;
      }

      if (!Array.isArray(items) || items.length === 0) {
        setError("No drugs could be identified in this image. Try better lighting, a closer photo, or make sure text isn't cut off at the edges.");
        setStage("upload");
        return;
      }
      setScanned(items);
      ...
      setError("Could not reach the scanning service. Check your internet connection and try again.");
      setStage("upload");
    }
  };
...
export default function App() {
  ...
  // Bottom nav items for mobile (only 5 fit nicely)
  const bottomNav = [
    { key: "dashboard", label: "Home", icon: "dashboard" },
    { key: "inventory", label: "Stock", icon: "inventory" },
    { key: "prescriptions", label: "Rx", icon: "prescription" },
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Inter', system-ui, sans-serif", background: COLORS.white, overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <header style={{ background: "#fff", borderBottom: `1px solid ${COLORS.border}`, padding: isMobile ? "12px 16px" : "14px 24px", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <h1 style={{ fontSize: isMobile ? 16 : 17, fontWeight: 700, color: COLORS.navy, margin: 0 }}>
              {tab === "more" ? "More Options" : navItems.find(n => n.key === tab)?.label}
            </h1>
          </div>
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
            {lowStockCount > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: 4, color: COLORS.amber, fontSize: 12, fontWeight: 600 }}>
                <Icon name="alert" size={14} /> {lowStockCount}
              </div>
            )}
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: COLORS.teal, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14 }}>A</div>
          </div>
        </header>
        ...
        <main style={{ flex: 1, overflowY: "auto", padding: isMobile ? "16px 14px" : 24, paddingBottom: isMobile ? 80 : 24 }}>
          {tab === "more" ? (
            <div>
              ...
              {moreItems.map(item => (
                <button key={item.key} onClick={() => setTab(item.key)} style={{
                  display: "flex", alignItems: "center", gap: 14, width: "100%", padding: "16px", marginBottom: 10,
                  background: "#fff", border: `1px solid ${COLORS.border}`, borderRadius: 12, cursor: "pointer", textAlign: "left"
                }}>
                  <div style={{ background: COLORS.tealLight, borderRadius: 10, padding: 10, color: COLORS.teal }}>
              ...
              <button onClick={resetAll} style={{ display: "flex", alignItems: "center", gap: 14, width: "100%", padding: "16px", marginTop: 20, background: COLORS.redLight, border: `1px solid #FECACA`, borderRadius: 12, cursor: "pointer" }}>
                <div style={{ background: "#FEE2E2", borderRadius: 10, padding: 10, color: COLORS.red }}>
                  <Icon name="x" size={20} />
                </div>
                <span style={{ fontSize: 15, fontWeight: 600, color: COLORS.red }}>Reset Sample Data</span>
              </button>
            </div>
          ) : renderContent()}
        </main>
      ...
      {isMobile && (
        <nav style={{
          ...
          {bottomNav.map(item => {
            const isActive = item.key === "more" ? tab === "more" || tab === "orders" || tab === "reports" : tab === item.key;
            return (
              <button key={item.key} onClick={() => setTab(item.key)} style={{
                flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                gap: 3, border: "none", background: "none", cursor: "pointer",
                color: isActive ? COLORS.teal : COLORS.slate, position: "relative"
              }}>
                <Icon name={item.icon} size={22} />
                <span style={{ fontSize: 10, fontWeight: isActive ? 700 : 400 }}>{item.label}</span>
                {item.key === "inventory" && lowStockCount > 0 && (
                  <span style={{ position: "absolute", top: 6, right: "calc(50% - 18px)", background: COLORS.red, color: "#fff", borderRadius: 99, padding: "0 5px", fontSize: 10, fontWeight: 700 }}>{lowStockCount}</span>
                )}
              </button>
            );
          })}
